                            ...
                            <tr class="center aligned" id="item_tr_edit_institu_admin{{$institution->id_institution}}">
                                <td>
                                    <div class="ui message error" style="display: none;"
                                         id="id_messagge_edit_institution{{$institution->id_institution}}">
                                        <ul class="list">
                                            <li id="id_messagge_error_p_edit_istitution{{$institution->id_institution}}"></li>
                                        </ul>
                                    </div>
                                    <input type="text" class="ui input" value="{{$institution->name_institution}}"
                                           placeholder="Nombre de la institución"
                                           id="name_institution_admin_edit{{$institution->id_institution}}"
                                           style="border: none;">
                                </td>
                                <td>
                                    <select class="ui dropdown"
                                            id="selec_edit_admin_sector{{$institution->id_institution}}">
                                        ...
                                        @else
                                            ...
                                            <option value="preescolar" selected="true">Preescolar, Básica y Media
                                            </option>
                                        @endif
                                    </select>
                                </td>
                                <td>
                                    <input type="text" class="ui input" value="{{$institution->telephone_institution}}"
                                           placeholder="Teléfono"
                                           id="name_telephone_amin_edit{{$institution->id_institution}}"
                                           style="border: none;">

                                <td>
                                    <select class="ui"
                                            id="value_city_institution_select_edit_admin{{$institution->id_institution}}">
                                        @foreach($ciudades as $ciudad)

                                            @if($institution->id_lugar_fk!=$ciudad->id_lugar)
                                                <option value="{{$ciudad->id_lugar}}">{{$ciudad->name_lugar}}</option>
                                            @else
                                                <option value="{{$ciudad->id_lugar}}"
                                                        selected="true">{{$ciudad->name_lugar}}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <select class="ui dropdown"
                                            id="inactive_institution_admin{{$institution->id_institution}}">
                                ...
                                <td class="collapsing">
                                    <button class="ui right floated small  labeled icon color_3 color_3 button"
                                            onclick="editInstitutionAdmin({{$institution->id_institution}})">
