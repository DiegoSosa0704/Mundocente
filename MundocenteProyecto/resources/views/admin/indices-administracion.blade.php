                            ...
                            <tr class="center aligned" id="id_level_aadmin_edit{{$index_g->id_level}}">
                                    ...
                                    <select class="ui dropdown" id="indice_admin_edit{{$index_g->id_level}}">
                                        @foreach($index as $ind)
                                            @if($index_g->id_index==$ind->id_index)
                                                <option value="{{$ind->id_index}}"
                                                        selected="true">{{$ind->name_index}}</option>
                                            @else
                                ...
                                <td>
                                    <div class="ui message error" style="display: none;"
                                         id="id_messagge_edit_index{{$index_g->id_level}}">
                                        <ul class="list">
                                            <li id="id_messagge_error_p_edit_index{{$index_g->id_level}}"></li>
                                        </ul>
                                    </div>
                                    <input type="text" id="value_level_index_paper_admin{{$index_g->id_level}}"
                                           value="{{$index_g->value_level}}" style="border: none;">
                                </td>
                                <td class="collapsing">
                                    <button class="ui right floated small  labeled icon color_3  color_3 button"
                                            onclick="edit_index_paper_admin({{$index_g->id_level}})">
