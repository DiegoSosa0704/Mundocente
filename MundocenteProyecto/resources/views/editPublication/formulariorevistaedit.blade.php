                        ...
                        <div class="ui form" id="form">
                                            ...
                                            @foreach($listInstitu as $inst_vin)
                                                ...
                                                @else
                                                    @if($publication_uni->id_institution==$inst_vin->id_institution)
                                                        <option value="{{$inst_vin->id_institution}}"
                                                                selected="true"> {{$inst_vin->name_institution}}</option>
                                                    @else
                            ...
                            <div class="field">
                                ...
                                <div class="ui inverted large horizontal label color_3">
                                    <div class="detail" id="name_city_title">
                                        Ciudad: {{$publication_uni->name_lugar}}</div>
                                </div>
                            </div>
                            <div class="field">
                                <div class="ui inverted large horizontal label color_2">
                                    <div class="detail" id="name_institute_title_select">
                                        Institución: {{$publication_uni->name_institution}}</div>
                                </div>
                            </div>
                            <div class="two fields" style="display: none;">
                                <div class="required field">
                                    <select class="ui search dropdown" name="country" disabled="true"
                                        ...
                                        @foreach($call_methods_theme->getCountryPublicationLugar($publication_uni->id_lugar_fk) as $pais_edit)
                                            <option value="{{$pais_edit->id_lugar}}"
                                                    selected="true">{{$pais_edit->name_lugar}}</option>
                                        @endforeach
                                ...
                                <div class="required field" id="cityChange">
                                    <select class="ui search dropdown" name="city" disabled="true"
                                        ...
                                        <option value="{{$publication_uni->id_lugar_fk}}" selected="true"
                                                id="enwcityselectedselect">{{$publication_uni->name_lugar}}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="required field">
                            ...
                            @if($call_methods_theme->returnIndexPublicationPaper($publication_uni->id_publication) > 0)
                                <div class="inline field">
                                    <input type="hidden" id="id_type_publication{{$publication_uni->id_publication}}"
                                           value="1">
                                    <label>
                                        No
                                    </label>
                                    <div class="ui toggle checkbox checked" onclick="showAdvancedSearch()">
                                        <input type="checkbox" name="indexed_paper" checked="true" tabindex="0"
                                               class="hidden"
                                               id="checkpaperindex">
                                    </div>
                                    <label>
                                                        ...
                                                        @foreach($clasificationpaper as $clasification)
                                                            ...
                                                            @if($clasification->id_index_fk==$index->id_index)
                                                                @if($call_methods_theme->getIndexPaper($publication_uni->id_publication, $clasification->id_level)==1)
                                                                    <option value="{{$clasification->id_level}}"
                                                                            selected="true">{{$clasification->value_level}}</option>
                                                                @else
                            ...
                            @endif
                            <input type="hidden" name="quantityindexlevel" id="idquantitypaperindex"
                                   value="{{$quantityIndex}}">
                                ...
                                <div class="required field">
                                    ...
                                    <select class="ui fluid search dropdown multiple" multiple="true"
                                        ...
                                        @foreach($listThemesPub as $theme_pub)
                                            <option value="{{$theme_pub->id_tema}}"
                                                    selected="true"> {{$theme_pub->name_theme}}
                                                - {{$theme_pub->type_theme}}</option>
                                        @endforeach
                            ...
                            <div class="field">
                                ...
                                @if($publication_uni->url_photo_publication != NULL)
                                    <div class="field">
                                        ...
                                        <img class="ui middle aligned medium rounded image"
                                             src="{{$publication_uni->url_photo_publication}}"
                                             id="imageNewShow">
                                        <span>
                                            <input type="hidden" name="imaTemp" id="imageAuxTemp"
                                                   value="{{$publication_uni->url_photo_publication}}">
                                    ...
                                    <div class="field">
                                        ...
                                        <span>
                                            <input type="hidden" name="imaTemp" id="imageAuxTemp" value="">
                                                <label for="file" class="ui blue button button_load">
                                                     Cargar
                                                     <form method="post" id="formularioimage"
